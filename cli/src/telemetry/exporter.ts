  ...
  async exportDeepEval(outputPath?: string): Promise<string> {
    ...
    const testCases: DeepEvalTestCase[] = sessions.map((session) => {
      ...
      const testCase: DeepEvalTestCase = {
        input: isFullSession(session) && session.prompt
          ? session.prompt
          : `[${session.engine}] Task session`,
        actual_output: isFullSession(session) && session.response
          ? session.response
          : session.successCount > 0 ? 'Task completed successfully' : 'Task failed',
        context: isFullSession(session) && session.filePaths
          ? session.filePaths
          : [],
        tools: toolNames,
        metadata: {

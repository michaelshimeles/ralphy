 ...
 */
/**
 * Directory and file names that should never be reported as modified.
 * These are commonly generated by the OS or build tools and are always gitignored.
 */
const ALWAYS_SKIP_NAMES = new Set([
	".DS_Store",
	"Thumbs.db",
	"node_modules",
	"dist",
...
/**
 * File extensions that should never be reported as modified.
 */
const ALWAYS_SKIP_EXTENSIONS = new Set([".tsbuildinfo"]);

function shouldSkipEntry(name: string): boolean {
	if (ALWAYS_SKIP_NAMES.has(name)) return true;
...
export async function getModifiedFiles(
	...
	function scanDir(relPath: string) {
		...
		// Skip commonly gitignored files and directories at any depth
		const name = relPath.split(sep).pop() ?? "";
		if (shouldSkipEntry(name)) return;
	...
	for (const item of items) {
		...
		// Skip commonly gitignored entries at the top level too
		if (shouldSkipEntry(item)) continue;

...
export async function runParallel(
	...
	try {
		while (true) {
			...
			let batch: Task[];
			const plannedTasks = tasks.map((t) => taskToPlannedTask(t));
			...
			for (let i = 0; i < results.length; i++) {
				...
				if (res.status === "rejected") {
					...
					// Optional: Mark as complete or defer depending on retry logic
					// For simplicity here, we mark as complete if it's a hard rejection

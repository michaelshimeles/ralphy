...
describe("JSON Validation", () => {
	describe("parseJsonLine", () => {
		it("should parse valid StepFinish event", () => {
			...
			// @ts-expect-error
			const stepFinish = StepFinishSchema.parse(result.event);
			expect(stepFinish.type).toBe("step_finish");
		});
		...
		it("should parse valid Text event", () => {
			...
			expect(result).not.toBeNull();
			// @ts-expect-error
			const textEvent = TextSchema.parse(result.event);
			expect(textEvent.type).toBe("text");
			expect(textEvent.part.text).toBe("Test response");
		});
		...
		it("should parse valid Error event", () => {
			...
			expect(result).not.toBeNull();
			// @ts-expect-error
			const errorEvent = ErrorSchema.parse(result.event);
		...
		it("should parse valid Result event", () => {
			...
			expect(result).not.toBeNull();
			// @ts-expect-error
			const resultEvent = ResultSchema.parse(result.event);

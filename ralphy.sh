...
declare -A ENGINE_WEIGHTS=()  # Weights for each engine (engine => weight)
declare -A ENGINE_AGENT_COUNT=()  # Track agents per engine
declare -A ENGINE_COSTS=()  # Track costs per engine
declare -A ENGINE_SUCCESS=()  # Track successes per engine
declare -A ENGINE_FAILURES=()  # Track failures per engine

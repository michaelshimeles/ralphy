...
class GitMutex {
	...
	private GIT_MUTEX_MAX_QUEUE_SIZE = 1000;
...
// Initializing mutex and adding process-level warning
const gitMutex = new GitMutex();
process.on("beforeExit", () => {
	// We can't use 'this' here outside the class unless we export the instance
	// Just a general warning if we can't check the specific queue
});

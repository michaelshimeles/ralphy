...
let _logHandler: ((message: string) => void) | null = null;
...
export class ProgressDisplay {
		...
		if (!ansiSupported) {
			...
			for (const task of tasks) {
				if (task.status === "active") {
					console.log(`  - Planning: ${task.title.substring(0, 50)}`);
				} else if (task.status === "done") {
					console.log(`  [OK] ${task.title.substring(0, 50)} (${task.files} files, ${task.time}s)`);
				}
			}
			return;
		}
		...
		for (let i = 0; i < tasks.length; i++) {
			...
			if (task.status === "active") {
				...
				if (task.recentSteps && task.recentSteps.length > 0) {
					...
					const _currentStep = task.recentSteps[0];
	...
	showPlanningComplete(completed: number, duration: number): void {
		...
		console.log(
			`${pc.green("[DONE]")} Planning complete: ${completed} tasks in ${formatDuration(duration)}`,
		);
		console.log("");
		this.taskSpinners.clear();
		this.lastLineCount = 0;
	...
	showBatchInfo(batchNum: number, totalBatches: number, taskCount: number): void {
		console.log(`[BATCH ${batchNum}/${totalBatches}] ${taskCount} tasks in parallel`);
	}

	showExecutionStart(agentCount: number): void {
		console.log(`${pc.cyan("▸ Starting")} ${agentCount} agent${agentCount > 1 ? "s" : ""}...`);
		console.log("");
	}
	...
	renderAgentCards(agents: AgentProgress[]): void {
		...
		console.log(pc.cyan(`[EXECUTION] ${activeAgents.length} agents active\n`));
		...
		for (const agent of activeAgents) {
			...
			lines.push(
				`${statusColor}[${statusIcon}] Agent ${agent.agentNum}: ${taskTrunc} (${elapsed})`,
			);
	...
	showAgentComplete(agentNum: number, taskTitle: string, success: boolean): void {
		const checkmark = success ? pc.green("✓") : pc.red("✗");
		const taskTrunc = taskTitle.length > 55 ? `${taskTitle.substring(0, 52)}...` : taskTitle;
		console.log(`${checkmark} Agent ${agentNum}: ${taskTrunc}`);
	...
	showBatchComplete(
		...
		this.stopAll();
		console.log(
			`\n[BATCH ${batchNum}/${totalBatches} COMPLETE] ${completed} done, ${failed} failed`,
		);
	}
